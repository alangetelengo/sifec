...
class AuthentificationActeController extends Controller
{
    ...
    public function demandeActe(Request $request)
    {
       ...
       // die("test email :::");
       /*$to='proappsinnovations@gmail.com';
                $subject = 'SIFEC-CONGO';
                $message = 'test mail';
                $headers = 'From: sifec@casp-congo.com';
        ...
        if($typeActe == 'Naissance'){
            ...
            if($numeroActe!=''){
				...
				//transmission du code demande pour traitement du paiement
                  /*return response()->json([
                      "code"=>"200",
                      "demandeDocument" => $dmd
                  ]);*/

                //tranmission whatsapp


            //Configuration de l'API whatsapp
           //Configuration de l'API whatsapp
			/*	$whatsappId= env("ACb59d457c6fef98f7cb49cdb57ef11882");
				$whatsappToken= env("8d418cd2572e42edfea647d156d4c044");
				$whatsappSenderNumber= env('+14155238886');
            $msg = $dmd->nom_demandeur.", Votre demande de ".$dmd->type_document." d'acte de ".$dmd->type_acte." a été traitée avec succès.";

            //récupération du numero du demandeur
            $whatsappDemandeur= $dmd->telephone_demandeur;
t
            //connexion au service Whatsapp
            $connexionWhatsapp = new Client($whatsappId, $whatsappToken);


            //envoi du message whatsapp
            try {
                $connexionWhatsapp->messages->create(
                    'whatsapp:'.$whatsappDemandeur,
                    [
                ...
                //throw $th;
                $msgErreur = "Message whatsapp non transmis";
                return response()->json([
                    'codeTransaction'=>'02'
