...
{
    /**
     * Sans portées globales (SoftDeletes, traits tiers comme AdjacencyList, etc.) :
     * toute ligne présente en base (y compris archivée / supprimée logiquement) occupe la PK.
     */
    private static function queryPourCodeReferentiel(Model $model)
    {
    ...
    {
        // Appels historiques du type (..., $taille, [], 'PREFIX_') : le préfixe utile est le 5e argument.
        if (is_array($prefix)) {
            $args = func_get_args();
            $prefix = (isset($args[4]) && is_string($args[4])) ? $args[4] : '';
        ...
        while ($attempt < $maxTries) {
            // Si l'appelant a déjà ouvert une transaction (ex. génération + insert dans le même flux),
            // ne pas committer ici : sinon les verrous sont libérés avant le save() et deux requêtes
            // concurrentes peuvent obtenir le même code (MySQL 1062).
            ...
            try {
                ...
                $maxObj = self::queryPourCodeReferentiel($model)
                    ->lockForUpdate()
                ...
                do {
                    ...
                    $exists = self::queryPourCodeReferentiel($model)
        ...
        try {
            ...
            // Vérifier que le code n'existe pas (toutes les lignes visibles sans portée métier)
            do {
                $strNumber = str_pad($max, $tailleString, '0', STR_PAD_LEFT);
                $code = $prefix.$strNumber;
                $exists = self::queryPourCodeReferentiel($model)->where($code_field, $code)->exists();
                if ($exists) {
                    $max++;
                }
    ...
    public static function asLetters($number, $separateur = ',')
    {
        ...
        $num[17] = ['0', 'un', 'deux', 'trois', 'quatre', 'cinq', 'six', 'sept', 'huit',
            'neuf', 'dix', 'onze', 'douze', 'treize', 'quatorze', 'quinze', 'seize'];

        $num[100] = [20 => 'vingt', 30 => 'trente', 40 => 'quarante', 50 => 'cinquante',
        ...
        try {
            ...
            while ($retryCount < $maxRetries) {
                try {
                    ...
                    $comDist = Localite::find($request->input('domicile_ville'.$sufix));
                    $comDist = $comDist ? $comDist->lib_localite : null;
                    $arrondissement = Localite::find($request->input('domicile_arrondissement'.$sufix));
                    $arrondissement = $arrondissement ? $arrondissement->lib_localite : null;
                    $quartier = Localite::find($request->input('domicile_quartier'.$sufix));
    ...
    public static function updatePersonne(Request $request, $sufix, $sexe, $code)
    {
        ...
        try {
            ...
            $adresseResidence = AdressePersonne::where('code_personne', $personne->code_personne)->first();
            if (! $adresseResidence) {
                $adresseResidence = new AdressePersonne;
