   ...
   function printDiv(divName) {
    ...
    $("#domicile_ville_declarant").on("change", function(){
        ...
        if(localiteParent != "" || localiteParent !=null){
            getQuartierVillage(localiteParent);
        }
        return false;
    });
     ...
     $("#domicile_ville_mere").on("change", function(){
        var localiteParent = $(this).val();
        if(localiteParent != "" || localiteParent !=null){
            //alert(localiteParent);
            getQuartierVillage2(localiteParent);
        }
        return false;
    });

     function getQuartierVillage(codeparent)
     {
         ...
         $.get(route, function (data) {
             ...
             option += +'</option>';
             $("#domicile_quartier_declarant").html(option);
         });
     }
     function getQuartierVillage2(codeparent)
     {
         var route = "{{ route('declarationNaissance.search.quartier.village',':id') }}";
         route = route.replace(":id", codeparent);
         var option = "<option> Selectionnez </option>";
         $.get(route, function (data) {
             ...
             option += +'</option>';
             $("#domicile_quartier_mere").html(option);
         });
     }
     ...
     $("#code_localite_enfant").change(function (e) {
            e.preventDefault();
            var code_localite_enfant = $(this).val();
            var lieunaisenfant = $("#code_localite_enfant option:selected").text();

            if(lieunaisenfant != null || lieunaisenfant != ''){
        ...
        $("#code_localite_pere").change(function (e) {
            e.preventDefault();
            var code_localite_pere = $(this).val();
            var lieunaispere = $("#code_localite_pere option:selected").text();

            if(lieunaispere != null || lieunaispere != ''){
        ...
        $("#code_localite_mere").change(function (e) {
            e.preventDefault();
            var code_localite_mere = $(this).val();
            var lieunaismere = $("#code_localite_mere option:selected").text();

            if(lieunaismere != null || lieunaismere != ''){
        ...
        $("#code_localite_declarant").change(function (e) {
            e.preventDefault();
            var code_localite_declarant = $(this).val();
            var lieunaisdeclarant = $("#code_localite_declarant option:selected").text();

            if(lieunaisdeclarant != null || lieunaisdeclarant != ''){
                $("#lieu_naissance_declarant").val(lieunaisdeclarant);
            }
        });


        $('#communeDistrict_pere').hide();
        $('#domicile_pays_pere').on('change', function () {
             var pays = $('#domicile_pays_pere').val();
             if (pays == 'Congo') {
                 $('#communeDistrict_pere').show();
                 $('#autreCommuneDistrict_pere').hide();
             } else {
                 $('#communeDistrict_pere').hide();
                 $('#autreCommuneDistrict_pere').show();
             }
         });

         $('#communeDistrict_mere').hide();
         $('#domicile_pays_mere').on('change', function () {
             var pays = $('#domicile_pays_mere').val();
             if (pays == 'Congo') {
                 $('#communeDistrict_mere').show();
                 $('#autreCommuneDistrict_mere').hide();
             } else {
                 $('#communeDistrict_mere').hide();
                 $('#autreCommuneDistrict_mere').show();
             }
         });

         $('#communeDistrict_declarant').hide();
         $('#domicile_pays_declarant').on('change', function () {
             var pays = $('#domicile_pays_declarant').val();
             if (pays == 'Congo') {
                 $('#communeDistrict_declarant').show();
                 $('#autreCommuneDistrict_declarant').hide();
             } else {
                 $('#communeDistrict_declarant').hide();
                 $('#autreCommuneDistrict_declarant').show();
             }
         });
         ...
         $('input:radio[name="autredeclarant"]').change(function()
         {
            ...
            if (($(this).val() === 'mere')&&($('#statut_personne_mere').val()==="VIVANT"))
            {
               ...
               //  alert($('#type_date_naissance_mere').val());
                 ...
                 // $('#lib_ville_declarant').val($('#domicile_ville_mere').val());
                 // document.getElementById('lib_ville_declarant').readOnly = true;
