                            ...
                            <table id="example" class="display" style="min-width: 845px">
                                ...
                                <tbody>
                                    @foreach ($declarations as $dn)
                                        {{-- @if($dn->mouvements->last()->statut == "Envoyée" || $dn->mouvements->last()->statut == "Renvoyée") --}}
                                        @if($dn->mouvements->last()->statut == "Envoyée")
                                            ...
                                            <tr width="100%">
                                                <td>
                                                    ...
                                                    <label for="single-check"><input type="checkbox" name="declarations[]" class="single-check" value="{{ $dn->code_declaration_naissance }}-{{ $dn->acte == null ? '0':'1' }}"></label>
                                                    @endif
                                                ...
                                                @if (!$annuler)
                                                    ...
                                                    @if($dn->mouvements->last()->statut == "Envoyée" && $dn->acte != null && $dn->acte->signature_mairie != null && $dn->acte->retirer == false)
                                                        <td style="width: 15%">
                                                            <a href="{{ $dn->acte->numeroActe->numero_acte }}" niupp="{{ $dn->acte->code_acte_naissance }}" class="badge badge-primary show-acte-retrait-modal">Acte produit non rétiré</a>
                                                        </td>
                                                    @endif
                                                    ...
                                                    <td style="width: 15%">
                                                        <div class="btn-group btn-group-xs">
                                                            ...
                                                            @endif
                                                            @if($dn->acte != null)
                                                                <a href="{{ route('acteNaissance.display',$dn->code_declaration_naissance) }}" target="_blank" class="btn btn-primary shadow btn-xs sharp me-1" title="Voir l'acte"><i class="fas fa-eye"></i></a>
    ...
    <script>
        ...
        $(function()
        {
         ...
         $("button.generate").on("click", function () {
            ...
            var code_declaration_naissance = $(this).attr("href");
            var url = "{{ route('acteNaissance.send.otp',':id') }}";
            url = url.replace(':id',code_declaration_naissance);
            ...
            $.get(url,function(response){
                if(response.code == "200"){
                    ...
                    $("#code_declaration_naissance").val(code_declaration_naissance)
