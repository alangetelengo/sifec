        ...
        <div class="mb-2 col-md-4">
            ...
            <select id="code_localite_pere" class="form-control">
                @foreach ($localites as $localite)
                    <option value="{{ $localite->code_localite }}" {{ $dn->pere->localite->code_localite == $localite->code_localite ? "selected" : "" }}>{{ $localite->lib_localite }}</option>
                @endforeach
            </select>
        </div>
         <div class="mb-2 col-md-4">
            ...
            <select id="code_nationalite_pere" name="code_nationalite_pere" class="form-control required  @error('code_nationalite_pere') is-invalid @enderror ">
                @foreach ($nationalites as $nationalite)
                    <option value="{{ $nationalite->code_nationalite }}" {{ $dn->pere->nationalite->code_nationalite == $nationalite->code_nationalite ? "selected" : "" }}>{{ $nationalite->lib_nationalite }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-2 col-md-4">
            ...
            <select id="profession_pere" class="form-control form-control wide">
                @foreach ($professions as $item)
                    <option value="{{ $item->code_profession }}" {{ $dn->pere->profession->code_profession == $item->code_profession ? "selected" : "" }}>{{ $item->lib_profession }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-2 col-md-4">
            ...
            <select id="niveau_instruction_pere" class="form-control form-control wide">
                @foreach ($instructions as $item)
                    <option value="{{ $item }}" {{ $dn->pere->niveau_instruction == $item ? "selected" : "" }}>{{ $item }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-2 col-md-4">
        ...
        <div class="row">
            ...
            <div class="mb-2 col-md-3 domicile_ville_pere">
                ...
                <select class="form-control" id="domicile_ville_pere">
                    @foreach ($localites as $item)
                        <option value="{{ $item->code_localite }}" {{  $dn->pere->adresses->last()->code_localite == $item->code_localite ? "selected" : "" }}>{{ $item->lib_localite }}</option>
                    @endforeach
                </select>
            </div>
            @if($dn->pere->adresses->last()->lib_pays != "Congo")
            <div class="mb-2 col-md-3 autredomicile_ville_pere">
                <label class="form-label">Ville<span class="text-danger"></span></label>
                <input type="text" id="autredomicile_ville_pere" class="form-control form-control wide" placeholder="Libellé de la ville" onkeyup="this.value=this.value.toUpperCase()" value="{{ $dn->pere->adresses->last()->lib_ville }}">
            </div>
            @endif
            <div class="mb-2 col-md-3 domicile_arrondissement_pere">
                ...
                <select class="form-control" id="domicile_arrondissement_pere">
                    @foreach ($arrondissement as $item)
                        <option value="{{ $item->code_localite }}" {{  $dn->pere->adresses->last()->code_arrondissement_comurbaine  == $item->code_localite ? "selected" : "" }}>{{ $item->lib_localite }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-2 col-md-3 domicile_quartier_pere">
                ...
                <select class="form-control" id="domicile_quartier_pere">
                    ...
                    @foreach ($quartierVillages as $localite)
                        <option value="{{ $localite->code_localite }}" {{  $dn->pere->adresses->last()->code_quartier_village  == $localite->code_localite ? "selected" : "" }}>{{ $localite->lib_localite }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-2 col-md-3">
                ...
                <select class="form-control" id="domicile_typevoie_pere">
                    {{-- <option value="">Choisir</option> --}}
                    <option value="Avenue" {{ $dn->pere->adresses->last()->type_voie == "Avenue" ? "selected" : "" }}>Avenue</option>
                    <option value="Boulevard" {{ $dn->pere->adresses->last()->type_voie == "Boulevard" ? "selected" : "" }}>Boulevard</option>
                    <option value="Impasse" {{ $dn->pere->adresses->last()->type_voie == "Impasse" ? "selected" : "" }}>Impasse</option>
                    <option value="Rue" {{ $dn->pere->adresses->last()->type_voie == "Rue" ? "selected" : "" }}>Rue</option>
                    <option value="Autre" {{ $dn->pere->adresses->last()->type_voie == "Autre" ? "selected" : "" }}>Autre</option>
                </select>
            </div>
            <div class="mb-2 col-md-3">
                <label class="form-label">N° voie<span class="text-danger"></span></label>
                <input type="text" class="form-control" id="domicile_numero_pere" placeholder="N° voie" value="{{ $dn->pere->adresses->last()->numero_rue }}">
            </div>
            <div class="mb-2 col-md-3">
                <label class="form-label">Nom voie<span class="text-danger"></span></label>
                <input type="text" class="form-control" id="domicile_nomvoie_pere" placeholder="Nom voie" style="text-transform: capitalize" value="{{ $dn->pere->adresses->last()->nom_voie }}">
            </div>
            ...
            <div class="row">
                <div class="col-md-4">
                    ...
                    <select name="code_pays_pere" id="code_pays_pere" class="form-control">
                        @forelse ($countries as $code)
                            <option value="{{ $code->dial_code }}" {{ $dn->pere->adresses->last()->lib_pays == $code->name ? "selected" : "" }}>({{ $code->dial_code }}) {{ $code->name }}</option>
                        @empty
                        @endforelse
                    </select>
                </div>
                ...
                <div class="mb-2 col-md-4">
                    ...
                    <input type="email" id="email_pere" class="form-control" name="email_pere" placeholder="Email père" value="{{ $dn->pere->user !='' ? $dn->pere->user->email : "" }}">
                </div>
...
<section>
    ...
    <div class="row adressemere">
        ...
        <div class="mb-2 col-md-3 domicile_ville_mere">
            <label class="form-label">Commune/District<span class="text-danger"></span></label>
            <select class="form-control" id="domicile_ville_mere">
                @foreach ($localites as $item)
                    <option value="{{ $item->code_localite }}" {{  $dn->mere->adresses->last()->code_localite == $item->code_localite ? "selected" : "" }}>{{ $item->lib_localite }}</option>
                @endforeach
            </select>
        </div>
        @if($dn->pere->adresses->last()->lib_pays != "Congo")
        ...
        <div class="mb-2 col-md-3 domicile_arrondissement_mere">
            ...
            <select class="form-control" id="domicile_arrondissement_mere">
                @foreach ($arrondissement as $item)
                    <option value="{{ $item->code_localite }}" {{  $dn->mere->adresses->last()->code_arrondissement_comurbaine  == $item->code_localite ? "selected" : "" }}>{{ $item->lib_localite }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-2 col-md-3 domicile_quartier_mere">
            ...
            <select class="form-control" id="domicile_quartier_mere">
                {{-- <option value="">Choisir</option> --}}
                @foreach ($quartierVillages as $localite)
                    <option value="{{ $localite->code_localite }}" {{  $dn->mere->adresses->last()->code_quartier_village  == $localite->code_localite ? "selected" : "" }}>{{ $localite->lib_localite }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-2 col-md-3">
    ...
    <div class="row">
        ...
        <div class="mb-2 col-md-4">
            ...
            <input type="number" name="nombre_enfants" min="0" class="form-control" placeholder="0" id="nombre_enfants" value="{{ $dn->nombre_enfant }}">
        </div>
        <div class="mb-2 col-md-4">
            <label class="form-label">Statut <span class="text-danger">*</span></label>
...
@include('naissance::adoption.adoptant')
