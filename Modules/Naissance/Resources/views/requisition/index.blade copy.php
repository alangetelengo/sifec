                                ...
                                <tbody>
                                    ...
                                    @isset($item->declarationNaissance)
                                        ...
                                        <tr width="100%">
                                            ...
                                            <td>{{ $item->declarationNaissance->type_declaration }} <strong style="color: red">N°: {{ $item->declarationNaissance->numero_certificat }}</strong></td>
                                            ...
                                            <td>
                                                <div class="btn-group btn-group-xs">
                                                    <a href="{{ route('declarationNaissance.etat',$item->declarationNaissance->code_declaration_naissance) }}" target="_blank" class="btn btn-warning shadow btn-xs sharp me-1" title="Voir certificat"><i class="fas fa-eye"></i></a>
                                                    ...
                                                    @can('module.fonctionnalites.requisitions')
                                                        ...
                                                        <a href="{{ $item->code_requisition }}" typejugmt="{{ $item->type_requisition }}" cec="{{ $item->code_institution }}" libelleCec="{{ $item->institution->lib_institution }}" cdn="{{ $item->declarationNaissance->code_declaration_naissance }}" numerojug="{{ $item->num_requisition }}" class="btn btn-info show-to-send shadow btn-xs sharp me-1" title="Envoyer" ><i class="fas fa-plane"></i></a>
                                                        @endif
                                                    @endcan
                                        ...
                                        @endif
                                    @endisset
                                    @isset($item->rectification)
                                        @if($item->statut == "En cours de traitement" || $item->rectification->statut = "Envoyé au tribunal")
                                        <tr width="100%">
                                            <td>
                                                {{$i++}}
                                            </td>
                                            <td>FICHE DE RECTIFICATION DE L'ACTE</td>
                                            <td>{{ strtoupper($item->type_requisition) ?? " -" }}</td>
                                            <td>{{ $item->num_requisition ?? " -" }}</td>
                                            <td>{{ $item->date_requisition ? date("d-m-Y", strtotime($item->date_requisition)) : " -" }}</td>
                                            <td>
                                            ...
                                            <td>
                                                <div class="btn-group btn-group-xs">
                                                    ...
                                                    {{-- @can('module.fonctionnalites.requisitions') --}}
                                                        @if($item->document_requisition != "")
                                                        <a href="{{ $item->code_requisition }}" cec="{{ $item->code_institution }}" libelleCec="{{ $item->institution->lib_institution }}" numrec="{{ $item->num_requisition }}" class="btn btn-info show-to-send shadow btn-xs sharp me-1" title="Envoyer" ><i class="fas fa-plane"></i></a>
                                                        @endif
                                                    {{-- @endcan --}}
                                                    @if($item->statut_document != "Envoyée")
                                                    <form  action="{{ route('requisition.destroy',$item->code_requisition) }}" method="POST" style="display: inline-block">
    ...
    <script>
        $(function(){
            $("a.show-to-send").on("click", function(){
                ...
                $("#numrequisition").html("Transmission de la "+typeJugmt+" N° "+numJugmt);
