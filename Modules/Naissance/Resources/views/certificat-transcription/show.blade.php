@extends('layout.app')
@section('titre')
Détail du certificat de transcription de l'acte
@endsection
@section('sous-titre')
Détail du certificat N° {{ $certificat->numero_certificat }} de transcription de l'acte
@endsection
@section('corps')
@php
    ...
    <div class="col-xl-12">
        ...
        <div class="mb-4 d-flex flex-wrap gap-2">
                ...
                if (isset($certificat->mouvements) && $certificat->mouvements && $certificat->mouvements->count()) {
                    ...
                    if (in_array($dernierMouvement->code_mouvement, ['MOUV_0019', 'MOUV_0015','MOUV_0006','MOUV_0011','MOUV_1019','MOUV_1001','MOUV_0031'])) {
                        $peutEnvoyer = false;
                        $peutModifier = false;
                    }
                }
            ...
            @if($peutModifier)
                @if($certificat->num_jugement_placement_provisoir != "" && $certificat->num_fiche_placement != "")
                    <button class="btn btn-primary btn-piece" data-type="extrait_main_courante" data-nom="" data-url="{{ route('declarationNaissance.piece.store', [$certificat->code_declaration_naissance, 'type' => 'extrait_main_courante']) }}" data-piece="{{ $certificat->piece_extrait_main_courante ??  $dummy }}" data-piece-url="{{ $certificat->piece_extrait_main_courante ? asset($certificat->piece_extrait_main_courante) : '' }}">
                        <i class="fa fa-file-alt"></i> Ajouter/Modifier l'extrait de la main courante
                    </button>
                @else
                    <button class="btn btn-primary btn-piece" data-type="declarant" data-nom="{{ $certificat->declarant->nom ??  $dummy }}" data-url="{{ route('declarationNaissance.piece.store', [$certificat->code_declaration_naissance, 'type' => 'declarant']) }}" data-piece="{{ $certificat->piece_declarant ??  $dummy }}" data-piece-url="{{ $certificat->piece_declarant ? asset($certificat->piece_declarant) : '' }}">
                        <i class="fa fa-id-card"></i> Ajouter/Modifier pièce Déclarant
                    </button>
                    <button class="btn btn-primary btn-piece" data-type="pere" data-nom="{{ $certificat->pere->nom ??  $dummy }}" data-url="{{ route('declarationNaissance.piece.store', [$certificat->code_declaration_naissance, 'type' => 'pere']) }}" data-piece="{{ $certificat->piece_pere ??  $dummy }}" data-piece-url="{{ $certificat->piece_pere ? asset($certificat->piece_pere) : '' }}">
                        <i class="fa fa-id-card"></i> Ajouter/Modifier pièce Père
                    </button>
                    <button class="btn btn-primary btn-piece" data-type="mere" data-nom="{{ $certificat->mere->nom ??  $dummy }}" data-url="{{ route('declarationNaissance.piece.store', [$certificat->code_declaration_naissance, 'type' => 'mere']) }}" data-piece="{{ $certificat->piece_mere ??  $dummy }}" data-piece-url="{{ $certificat->piece_mere ? asset($certificat->piece_mere) : '' }}">
            ...
            @if(Auth::user()->affectationActive()->code_institution == $certificat->code_institution_destinataire)
                ...
                @if($peutEnvoyer)
                    <button class="btn btn-warning btn-envoyer-centre{{ $peutEnvoyer ? '' : ' disabled' }}"
                        ...
                        title="Envoyer le dossier au tribunal"
            ...
            <div class="card-body">
                <table class="table table-bordered table-striped align-middle mb-0">
                    <tbody>
                        ...
                        <tr>
                            <th><i class="fa fa-baby text-primary me-1"></i> Enfant</th>
                            <td>{{ $certificat->enfant ? $certificat->enfant->nomcomplet() : '-' }}</td>
                        </tr>
                        <tr>
                            <th><i class="fa fa-baby text-primary me-1"></i> Sexe</th>
                            <td>{{ $certificat->enfant ? $certificat->enfant->sexe == 'M' ? 'Masculin' : 'Féminin' : '-' }}</td>
                        </tr>
                        <tr>
                            <th><i class="fa fa-calendar text-primary me-1"></i> Date de naissance</th>
                            <td>{{ isset($certificat->date_heure_naissance) ? date('d/m/Y H:i', strtotime($certificat->date_heure_naissance)) : '-' }} minute(s)</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td>{{ $certificat->lieuSurvenance->lib_lieu_survenance ?? $dummy }}</td>
                        </tr>
                        <tr>
                            <th><i class="fa fa-user-tie text-primary me-1"></i> Déclarant</th>
                            <td>{{ $certificat->declarant ? $certificat->declarant->nomcomplet() : '-' }}</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td>{{ $certificat->pere ? $certificat->pere->nomcomplet() : '-' }}</td>
                        </tr>
                        <tr>
                            <th><i class="fa fa-female text-primary me-1"></i> Mère</th>
                            <td>{{ $certificat->mere ? $certificat->mere->nomcomplet() : '-' }}</td>
                        </tr>
                ...
                <div class="mt-4">
                    ...
                    <a href="{{ route('certificatTranscription.index') }}" class="btn btn-info float-end"> <i class="fa fa-list"></i> Retour à la liste</a>
                    @endif
                </div>
            </div>
        </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="alert alert-info">
                        Cette action va transmettre le dossier au tribunal pour une demande de réquisition.<br>
