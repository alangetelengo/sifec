...
$(function(){
    ...
    $("#modalListePiece").on('click', function(){
       alert("ok");
    });


    //CAS DE JUGEMENT HOMOLOGATION--RECHERCHE DE L'ACTE ANNULE


    $("#rechercherEnfant").on("click", function(event){
        event.preventDefault();
        numero_acte_naissance = $("#numero_acte_naissance_enfant").val();
        if(numero_acte_naissance == ""){
            flashAlert("Opération échouée","error","Veuillez saisir le numéro de l'acte de naissance");
            return false;
        }
        var data = {
        ...
        if(response.code == "200"){
            ...
            //vider le input
            $("#numero_acte_naissance_enfant").val("");

            $(".search-enfant-modal-lg").modal("hide");
            //identité de l'enfant
            $("#numero_ancien_acte").val(response.numero_ancien_acte);
            $("#nom_enfant").val(response.nom);
            $("#prenom_enfant").val(response.prenom);
            $("#sexe_enfant").val(response.sexe);
            $("#heure_naissance_enfant").val(response.date_heure_naissance);
            $("#date_naissance_enfant").val(response.date_naissance);
            $("#date_heure_declaration").val(response.date_heure_declaration);
            $("#lieu_naissance_enfant").val(response.lieu_naissance);
            $("#num_acte_naissance_enfant").val(numero_acte_naissance);
            $("#code_nationalite_enfant").val(response.lib_nationalite);
            $("#date_emission_acte_naissance_enfant").val(response.dateEmisAN);
            $("#code_lieu_survenance").val(response.code_lieu_survenance);
            $("#code_profession_enfant").val(response.code_profession);
            $("#cec_enfant").val(response.cec_naissance);
            $("#code_situation_matrimoniale").val(response.code_situation_matrimoniale);

            //identité de la mère
            $("#nom_mere").val(response.nom_mere);
            $("#prenom_mere").val(response.prenom_mere);
            $("#sexe_mere").val(response.sexe_mere);
            $("#heure_naissance_mere").val(response.date_heure_naissance_mere);
            $("#date_naissance_mere").val(response.date_naissance_mere);
            $("#date_heure_declaration").val(response.date_heure_declaration_mere);
            $("#code_localite_mere").val(response.lieu_naissance_mere);
            $("#num_acte_naissance_mere").val(numero_acte_naissance_mere);
            $("#code_nationalite_mere").val(response.lib_nationalite_mere);
            $("#date_emission_acte_naissance_mere").val(response.dateEmisAN_mere);
            $("#code_lieu_survenance").val(response.code_lieu_survenance_mere);
            $("#code_profession_mere").val(response.code_profession_mere);
...
//FIN CAS DE JUGEMENT HOMOLOGATION--RECHERCHE DE L'ACTE ANNULE
    ...
    function soumission()
    {
        ...
        for (var index = 1;  nombre_temoin >= index; index++) {
         ...
         var domicile_nomvoie_mere = $("#domicile_nomvoie_declarant");
        ...
        var num_jugement = $("#num_jugement").val();
        var date_jugement = $("#date_jugement").val();
        var numero_ancien_acte = $("#numero_ancien_acte").val();
        ...
        var data =
        {
            ...
            numero_ancien_acte:numero_ancien_acte,
...
+"<tr><td style='font-weight:bold; padding:2px'></td><td style='padding:2px'>Adresse<br><span style='font-weight:bold;'>"+domicile_numero_declarant.val()+" "+domicile_nomvoie_declarant.val()+" "+$("#domicile_ville_declarant option:selected").text()+" "+$("#domicile_arrondissement_declarant option:selected").text()+" "+$("#domicile_quartier_declarant option:selected").text()+"</span></td><td style='padding:2px'>Filiation<br><span style='font-weight:bold;'>"+document.getElementById( "filiation" ).options[ document.getElementById( "filiation" ).selectedIndex ].text +"</span></td><td style='padding:2px'>Téléphone<br><span style='font-weight:bold;'>"+ telephone_declarant.val()+"</span></td><td style='padding:2px'>Profession<br><span style='font-weight:bold;'>"+document.getElementById( "profession_declarant" ).options[ document.getElementById( "profession_declarant" ).selectedIndex ].text +"</span></td><td style='padding:2px'>Nationalite<br><span style='font-weight:bold;'>"+document.getElementById( "code_nationalite_declarant" ).options[ document.getElementById( "code_nationalite_declarant" ).selectedIndex ].text+"</span></tr>"
         ...
         }).then((result)=>
          {
            ...
            {
                $.post("{{route('declarationNaissance.store.jugement.homologation')}}",data,function(response)
                {
                    if(response.code == "200")
                    {
