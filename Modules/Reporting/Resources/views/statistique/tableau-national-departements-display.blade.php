@extends('layout.app')
@section('titre')
Tableau national par département — {{ $annee }}
@endsection
@section('corps')
<div class="row">
    <div class="col-xl-12">
        <div class="card">
            <div class="card-header">
                <h4>Tableau statistique national — {{ $annee }}</h4>
                <div class="row">
                    <div class="d-flex flex-wrap align-items-center gap-2 justify-content-end w-100 px-1">
                        <a href="{{ route('reporting.statistique.tableau.national.departements') }}" class="btn btn-sm btn-info text-white mb-2">
                            <i class="fas fa-arrow-left me-1"></i> Retour au formulaire
                        </a>
                        <button type="button" class="btn btn-sm btn-success mb-2" id="btn-exporter-tableau" title="Exporter en PDF">
...
function sifecExportAuthenticatedPdf(url, filename) {
    fetch(url, { credentials: 'same-origin', headers: { 'Accept': 'application/pdf,*/*' } })
        ...
        .then(function (blob) {
            ...
            link.href = blobUrl;
            link.download = filename || 'tableau-national-departements.pdf';
...
$(function () {
    ...
    $('#btn-exporter-tableau').on('click', function () {
