    ...
    <div class="col-xl-12">
        <div class="card">
            <div class="card-header">
                <h4>Annuaire statistique — Naissances vivantes enregistrées</h4>
                <div class="row">
                    <div class="d-flex flex-wrap align-items-center gap-2 justify-content-end w-100 px-1">
                        <a href="{{ route('reporting.naissance.annuaire.statistique') }}" class="btn btn-sm btn-info text-white mb-2">
...
function sifecExportAuthenticatedPdf(url, filename) {
    fetch(url, { credentials: 'same-origin', headers: { 'Accept': 'application/pdf,*/*' } })
        ...
        .then(function (blob) {
            ...
            link.href = blobUrl;
            link.download = filename || 'annuaire-statistique-naissances.pdf';
...
$(function () {
    ...
    $('#btn-exporter-annuaire').on('click', function () {
        sifecExportAuthenticatedPdf(route, 'annuaire-statistique-naissances.pdf');
    });
});
</script>
@endsection
