    ...
    <script>
        ...
        $(".tab-wizard").steps({
            ...
            onFinished: function (event, currentIndex) {
                Swal.fire("Certificat Enrégistré !", "dossier est en cours de traiatement, consectetur adipiscing elit. Sed lorem erat eleifend ex semper, lobortis purus sed.");
...
if(($('#code_situation_matrimoniale_defunt').val()==="SMAT_0001"))
{
    ...
    conjoint="";
                ...
                Swal.fire({
                    width:2500,
                    position: 'top',
                    title: "Enrégistrer le certificat de transcription de décès ?",
                    icon: 'question',
    ...
    +"<tr><td style='padding:5px;' colspan=11><hr style='border:none;'></td></tr></table><bR><br>Assurez-vous, puis confirmez !</div> ",
                ...
                }).then(function (e)
                {
                    ...
                    if (e.value === true)
                    {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: "{{route('certificatTranscriptionDeces.store')}}",
                            data: {
                           ...
                           // data: {_token: token},
                            success: function(response )
                            {
                                ...
                                {
                                    swal.fire("Enrégistré!", response.message, "success");
                                    var url= "{{ route('certificatTranscriptionDeces.index') }}";
                                    setTimeout(() => {
                                        window.open(url);
                                    }, 4000);
                                } else {
