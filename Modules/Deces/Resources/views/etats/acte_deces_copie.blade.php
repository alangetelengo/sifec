...
{{-- <page orientation="portrait" backimg="" backcolor="#FEFEFE" backimgx="center" backimgy="100%" backimgw="100%" backtop="0"  backbottom="30mm" style="font-size: 12pt"> --}}
    ...
    <page_footer>
        @include('partials.guot.mention-legale-pied', ['typeDocument' => 'copie_deces'])
    </page_footer>
    @php
    setlocale(LC_TIME, "fr_FR", "French");
        ...
        // Règle métier : la copie n'est signée que sur demande de l'intéressé, par l'officier en
        // cours de fonction (flux « Demande de document » → copieActeDecesDemande). Hors demande,
        // ni QR (preuve de signature) ni signature ne s'affichent.
        ...
        if($acte->declaration->type_declaration == "CERTIFICAT DE NON INSCRIPTION"){
            $infos = 'ACTE ETABLIT SUIVANT LA REQUISITION AUX FINS DE DECLARATION TARDIVE N° '.$acte->declaration->requisition->num_requisition.' DU '.(date("d-m-Y", strtotime($acte->declaration->requisition->updated_at)))." AU ".$acte->declaration->requisition->institutionUser->institution->lib_institution;
        }
        ...
        $institutionActe = $acte->declaration->institutionUser->institution;
        $localisationData = \App\Sifec\Sifec::getLocalisationInstitution($institutionActe);
        
        $commune = $localisationData['localite'];
        $dept = $localisationData['localiteParent'];
        $localisationActe = $localisationData['localisation'];
    @endphp
            ...
            <tbody>
                <tr>
                    ...
                    <td style="text-align: left;">
                       ...
                       <p>Fait à {{ ucfirst(strtolower($localisationActe)) }}@if($estSigneDelivrance), le {{utf8_encode(strftime("%d %B %Y", strtotime( $acte->date_emission)))}}@endif</p>
                       ...
                       {{-- @if ($acte->approbation_pompe_funebre != "")
                        {{ $acte->signataire->user->affectationActive()->fonction->lib_fonction }}
                         <p>L'officier de l'état civil</p>
