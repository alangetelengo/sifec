...
class Institution extends Model
{
    ...
    public function getDeclarationsFormationSanitaireAControler($module)
    {
        ...
        if($module == "naissance"){
            ...
            ->whereIn('code_institution', $pompesFunebresCodes)
    ...
    public function getDeclarationsCentreEtatCivilApprouvees($module)
    {
        if($module == "naissance"){
            ...
            $typeDeclaration = "DECLARATION DE DECES";
            return DeclarationDeces::with(['defunt', 'declarant', 'pere', 'mere', 'mouvements', 'acte'])
            ->where('code_institution', $this->code_institution)
            ->where('type_declaration', $typeDeclaration)
            ->where('cec_approuver', 'OUI')
            ->get();
        }elseif($module == "mariage"){
    ...
    public function getActesGestion($module)
    {
        ...
        // 1. Déclarations approuvées des formations sanitaires
        $declarationsFormationSanitaire = $this->getDeclarationsFormationSanitaireApprouvees($module);
        if ($declarationsFormationSanitaire->isNotEmpty()) {
            $actesGestion = $actesGestion->merge($declarationsFormationSanitaire);
        }

        // 2. Déclarations avec réquisitions/jugements approuvés
        $declarationsJudiciaires = $this->getDeclarationsWithRequisitionsOrJugements($module);
        if ($declarationsJudiciaires->isNotEmpty()) {
            $actesGestion = $actesGestion->merge($declarationsJudiciaires);
        }

        // 3. Déclarations directes du centre approuvées
        $declarationsCentre = $this->getDeclarationsCentreEtatCivilApprouvees($module);
        if ($declarationsCentre->isNotEmpty()) {
            $actesGestion = $actesGestion->merge($declarationsCentre);
        }
