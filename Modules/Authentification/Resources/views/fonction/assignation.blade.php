...
@php
    $selectedTotal = count($assignedCodes);
@endphp
...
<div class="container-fluid page-ap-sifec">
    <nav class="ap-breadcrumb" aria-label="Fil d'Ariane">
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item"><a href="{{ route('fonction.index') }}">Fonctions</a></li>
            <li class="breadcrumb-item active" aria-current="page">Assignation</li>
        </ol>
    </nav>
    ...
    <div class="ap-shell">
            ...
            <div class="ap-hero-inner">
                <div class="d-flex flex-wrap justify-content-between align-items-start gap-3">
                    <div class="flex-grow-1" style="min-width: 220px;">
                        <h1>
                            <span class="ap-hero-icon" aria-hidden="true"><i class="fas fa-briefcase"></i></span>
                            Fonctionnalités par module
                        </h1>
                        <div class="ap-hero-meta">
                            <span><i class="fas fa-tag"></i> {{ $fonction->lib_fonction }}</span>
                            <span><i class="fas fa-fingerprint"></i> {{ $fonction->code_fonction }}</span>
                        </div>
                        <div class="ap-chips">
                            <span class="ap-chip"><i class="fas fa-check-circle"></i> <strong>{{ $selectedTotal }}</strong> / {{ $totalFonctionnalites }} fonctionnalités cochées</span>
                            <span class="ap-chip"><i class="fas fa-layer-group"></i> <strong>{{ $modules->count() }}</strong> modules</span>
                        </div>
                    </div>
                    <a href="{{ route('fonction.index') }}" class="btn-ap-ghost align-self-start">
                        <i class="fas fa-arrow-left"></i> Liste des fonctions
                    </a>
                </div>
            </div>
        </div>
        ...
        <div class="ap-info" role="note">
            ...
            <span>
                Les cases cochées définissent les <strong>permissions de base</strong> pour la fonction « {{ $fonction->lib_fonction }} ». Tout agent ayant cette fonction en bénéficie ; des droits supplémentaires peuvent encore être ajoutés par utilisateur (écran assignation permissions).
            </span>
        </div>
        ...
        <form action="{{ route('fonction.assigner.store', $fonction->code_fonction) }}" method="POST" id="assignationForm">
            ...
            @else
                <div class="accordion perm-accordion" id="accordion-fonction-permissions">
                    @foreach ($modules as $module)
                        @php
                            ...
                            $selMod = $codesInModule->filter(function ($c) use ($assignedCodes) {
                                return in_array($c, $assignedCodes, true);
                            })->count();
                                        ...
                                        <div class="perm-parent-row">
                                            <div class="form-check d-flex align-items-start">
                                                <input class="form-check-input permission-checkbox parent-checkbox"
                                                       ...
                                                       {{ in_array($parent->code_fonctionnalite, $assignedCodes, true) ? 'checked' : '' }}
                                            ...
                                            @foreach ($children as $child)
                                                <div class="perm-child-item">
                                                    <input class="form-check-input permission-checkbox child-checkbox"
                                                           ...
                                                           {{ in_array($child->code_fonctionnalite, $assignedCodes, true) ? 'checked' : '' }}>
            ...
            <div class="ap-footer">
                <a href="{{ route('fonction.index') }}" class="btn-ap-cancel">
                    <i class="fas fa-times"></i> Annuler
                </a>
                <button type="submit" class="btn-ap-save" id="assignationSubmitBtn">
                    <i class="fas fa-save"></i> Enregistrer les fonctionnalités
                </button>
            </div>
        </form>
    </div>
...
function setAllAccordionExpanded(expand) {
    document.querySelectorAll('#accordion-fonction-permissions .accordion-collapse').forEach(function (el) {
        if (expand) {
    ...
    document.querySelectorAll('#accordion-fonction-permissions .accordion-button').forEach(function (btn) {
